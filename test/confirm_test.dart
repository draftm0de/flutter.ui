...
void main() {
  group('DraftModeUIConfirm.show', () {
    testWidgets('shows Material dialog with localized labels and returns selection', (tester) async {
    ...
    testWidgets('auto confirm countdown uses localization and resolves after timer', (tester) async {
      await _withTargetPlatform(TargetPlatform.android, () async {
      final result = ValueNotifier<bool?>(null);
      ...
      expect(find.textContaining('Automatically confirms in 00:02'), findsOneWidget);
    ...
    testWidgets('cupertino dialog path renders CupertinoAlertDialog', (tester) async {
    ...
    testWidgets('auto confirm duration <= 0 resolves immediately on next frame', (tester) async {
      await _withTargetPlatform(TargetPlatform.android, () async {
      final result = ValueNotifier<bool?>(null);
    ...
    testWidgets('falls back to default strings when localization context missing', (tester) async {
      await _withTargetPlatform(TargetPlatform.android, () async {
      final result = ValueNotifier<bool?>(null);
      ...
      expect(find.text('Yes'), findsOneWidget);
      expect(find.text('No'), findsOneWidget);
      expect(find.textContaining('Automatically confirms in 00:01'), findsOneWidget);
      });
    });
  });
}

Future<void> _withTargetPlatform(TargetPlatform platform, Future<void> Function() body) async {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      localizationsDelegates:
          provideLocalization ? AppLocalizations.localizationsDelegates : const <LocalizationsDelegate<dynamic>>[],
      supportedLocales: provideLocalization ? AppLocalizations.supportedLocales : const <Locale>[Locale('en')],
      home: home,
    );
  }
}
